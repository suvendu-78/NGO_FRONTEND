...
//   // 🔒 FORM VALIDATION (controls button enable/disable)
//   const isFormValid = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-950 via-black to-gray-900 p-6 flex items-center justify-center">
      ...
      <div className="max-w-5xl w-full grid md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 space-y-6">
          {/* DETAILS */}
          <div className="bg-white/10 backdrop-blur-xl rounded-2xl p-6 border border-white/20 text-white">
            <h2 className="text-xl font-bold mb-6 flex items-center gap-2">
              <User className="text-amber-400" />
              Delivery / Orderer Details
            </h2>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              ...
              />
              <textarea rows={3} className="input md:col-span-2"
                placeholder="Full Delivery Address"
                value={details.address}
            ...
            {method==="netbank" && (
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                {["sbi","hdfc","icici","axis","kotak"].map(bank=>(
                  <div key={bank}
                    className="bg-white/10 rounded-xl p-4 flex flex-col items-center hover:bg-white/20 cursor-pointer">
        ...
        {/* SUMMARY */}
        <div className="bg-white/10 backdrop-blur-xl rounded-2xl p-6 border border-white/20 text-white h-fit">
          <h3 className="text-xl font-bold mb-4">🧾 Order Summary</h3>
          ...
          <div className="space-y-2 text-sm">
            ...
            <hr className="border-white/20"/>
            <div className="flex justify-between font-bold text-lg text-amber-400">
              <span>Total</span><span>₹{(total+gst).toFixed(2)}</span>
            </div>
          </div>
          ...
          <button
            ...
            className={`mt-6 w-full py-3 rounded-xl font-semibold flex items-center justify-center gap-2
              ${isFormValid()
                ? "bg-gradient-to-r from-amber-600 to-amber-800 hover:scale-105"
...
const Tab = ({ icon, label, active, onClick }) => (
  <button onClick={onClick}
    className={`p-4 rounded-xl border flex flex-col items-center
      ${active ? "bg-amber-700/30 border-amber-500" : "bg-white/5 border-white/20"}`}
  >
    {icon}
    <span className="text-sm mt-2">{label}</span>
  </button>
