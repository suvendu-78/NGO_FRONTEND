...
const BookReader = () => {
  ...
  if (!book) {
    return (
      <div className="min-h-screen flex items-center justify-center text-base sm:text-lg">
        No Book Found ❌
      </div>
    );
  }
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-100">
      ...
      >
        <button
          onClick={() => navigate(-1)}
          className="flex items-center gap-1 sm:gap-2 text-gray-700 text-sm sm:text-base"
        >
          <ArrowLeft size={18} />
          <span className="hidden sm:inline">Back</span>
        </button>

        <div className="flex-1 text-center font-medium text-sm sm:text-base truncate px-2">
          {book.title}
        </div>
